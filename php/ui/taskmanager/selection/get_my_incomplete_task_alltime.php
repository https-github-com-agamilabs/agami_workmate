...
try {
    ...
    $results = get_my_incomplete_task($dbcon, $userno);
    $results_array = array();
...
function get_my_incomplete_task($dbcon, $userno)
{
    $sql = "SELECT channelno,(SELECT channeltitle FROM msg_channel WHERE channelno=b.channelno) as channeltitle,
                    ...
                    relativepriority,howto,assignedto,assigntime,scheduledate,duration,
                    s.cblscheduleno,
                    b.userno, (SELECT CONCAT(firstname,' ',IFNULL(lastname,'')) FROM hr_user WHERE userno=b.userno) as assignedby
            FROM
                (SELECT cblscheduleno,backlogno,howto,assignedto,assigntime,scheduledate,duration, userno
