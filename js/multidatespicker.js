...
function loadControl(month, year) {
    ...
    var tbl = document.querySelector("#calendar_table_body");
    ...
    let validMonthStart = currentDate.getMonth();
    let validDateStart = currentDate.getDate();
    let date = 1;
    ...
    for (let rowIterator = 0; rowIterator < 6; rowIterator++) {
        ...
        for (let cellIterated = 0; cellIterated < 7 && date <= daysInMonth(month, year); cellIterated++) {
            ...
            if (rowIterator !== 0 || cellIterated >= firstDay) {
                ...
                if (month >= validMonthStart && date >= validDateStart) {
                    cell.class = "clickable";
                } else {
                    cell.style = 'pointer-events:none;background-color:#cccccc;';
                }
                textNode = date;
    ...
    $("#calendar_table_body tr td").click(function (e) {
        var id = $(this).attr('id');
        ...
        var sortedArray = selectedDates.sort((a, b) => new Date(a) - new Date(b));

        // update the leave_dates_input text input
        document.getElementById('leave_dates_input').value = datesToString(sortedArray);
    });


    var $search = $('#leave_dates_input');
    var $dropBox = $('#parent');
...
// https://www.geeksforgeeks.org/how-to-get-the-number-of-days-in-a-specified-month-using-javascript/?ref=lbp
