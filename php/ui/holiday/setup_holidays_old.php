...
try {
    ...
    if ($hdtypeid == "WEEK_END") {
        ...
        if (isset($_POST['delete_previous_weekends']) && $_POST['delete_previous_weekends'] && $current_holidays->num_rows > 0) {
...
function insert_holiday($dbcon, $holidaydate, $reasontext, $hdtypeid)
{
    $sql = "INSERT INTO emp_holidays
            (holidaydate, reasontext, hdtypeid)
            VALUES (?,?,?)";
    $stmt = $dbcon->prepare($sql);
    if ($dbcon->error) {
        echo $dbcon->error;
    }
    $stmt->bind_param("sss", $holidaydate, $reasontext, $hdtypeid);
    $stmt->execute();
